...
from poi_importer import (
    ...
    build_mapping,
    merge_poi_data,
    transform_source_to_poi,
)

try:
    from feishu_poi_sync import sync_feishu_pois
except Exception:
    sync_feishu_pois = None
...
def inject_app_theme():
    ...
    st.markdown(
        ...
        .hzh-poi-col-name { width: 210px; }
        .hzh-poi-col-feature { width: 260px; }
        .hzh-poi-col-category { width: 130px; }
        .hzh-poi-col-district { width: 120px; }
        .hzh-poi-col-fit { width: 280px; }
        .hzh-poi-col-budget { width: 175px; }
        .hzh-poi-col-address { width: 260px; }
        .hzh-poi-col-duration { width: 105px; }
        .hzh-poi-col-currency { width: 90px; }
...
DEFAULT_FEISHU_WIKI_URL = "https://my.feishu.cn/wiki/ByyMwEWN0iPtVXksSDtcgjJfnLg?from=from_copylink"
DEFAULT_FEISHU_TABLE_ID = "tblNnZpZDhlTHAiU"
DEFAULT_FEISHU_VIEW_ID = "vewt5rgo06"
...
def feishu_configured():
    return all([
        secret_or_env("FEISHU_APP_ID"),
        secret_or_env("FEISHU_APP_SECRET"),
        secret_or_env("FEISHU_APP_TOKEN") or secret_or_env("FEISHU_WIKI_URL") or DEFAULT_FEISHU_WIKI_URL,
        secret_or_env("FEISHU_TABLE_ID") or DEFAULT_FEISHU_TABLE_ID,
    ])


def clean_feishu_visible_text(value):
    """Clean Feishu visible text and remove option-id leftovers such as pure numbers."""
    text = clean_text(value)
    if not text:
        return ""
    parts = []
...
def feishu_poi_fixed_mapping(source_columns):
    """Map the current Feishu POI master table fields to the internal POI schema."""
    available = {clean_text(column): clean_text(column) for column in source_columns}
    desired = {
        "poi_id": "飞书记录ID",
        "code": "飞书记录ID",
        "name_cn": "POI/服务名称",
        ...
        "currency": "预算单位",
        "tags": "体验特色",
        "suitable_for": "医旅适配及注意事项",
        "description": "体验特色",
        "description_cn": "体验特色",
        "recommended_reason": "医旅适配及注意事项",
        "highlights": "体验特色",
        "address": "地址",
        "notes": "医旅适配及注意事项",
        "internal_remark": "医旅适配及注意事项",
    }
    return {target: available[source] for target, source in desired.items() if source in available}
...
def feishu_budget_text(row):
    """Render Feishu-style reference budget from internal cost fields."""
    min_price = optional_number(row.get("price_min"))
    max_price = optional_number(row.get("price_max"))
    currency = clean_text(row.get("currency")) or "CNY"
    if min_price is None and max_price is None:
        return "暂无"
    if min_price is not None and max_price is not None and min_price != max_price:
        return f"{currency} {min_price:,.0f} - {max_price:,.0f}"
    value = min_price if min_price is not None else max_price
    return f"{currency} {value:,.0f}"


def render_poi_list_table(poi_df, filtered_df):
    """Render a fixed-width, horizontally scrollable POI list using the Feishu master table fields."""
    ...
    for _, row in filtered_df.iterrows():
        ...
        duration_text = f"{duration:g}" if duration is not None else "暂无"
        feature_text = clean_feishu_visible_text(row.get("tags")) or clean_feishu_visible_text(row.get("highlights")) or clean_feishu_visible_text(row.get("description_cn"))
        fit_text = clean_feishu_visible_text(row.get("suitable_for")) or clean_feishu_visible_text(row.get("notes")) or clean_feishu_visible_text(row.get("internal_remark"))
        ...
        rows_html.append(
            ...
            f"<td class='hzh-poi-col-name'>{table_cell(row.get('name_cn'))}</td>"
            f"<td class='hzh-poi-col-feature'>{table_cell(feature_text)}</td>"
            f"<td class='hzh-poi-col-category'>{table_cell(row.get('category'))}</td>"
            f"<td class='hzh-poi-col-district nowrap'>{table_cell(row.get('district'))}</td>"
            f"<td class='hzh-poi-col-fit'>{table_cell(fit_text)}</td>"
            f"<td class='hzh-poi-col-budget nowrap'>{html.escape(feishu_budget_text(row))}</td>"
            f"<td class='hzh-poi-col-address'>{table_cell(row.get('address'))}</td>"
            f"<td class='hzh-poi-col-duration nowrap'>{html.escape(duration_text)}</td>"
            f"<td class='hzh-poi-col-currency nowrap'>{table_cell(row.get('currency'), 'CNY')}</td>"
    ...
    table_html = """
    <div class="hzh-poi-list-note">列表字段已按飞书「城市 POI 汇总表」展示；详情修改与图片管理请进入单个 POI 编辑页。</div>
    <div class="hzh-poi-table-wrapper">
      <table class="hzh-poi-table">
        <colgroup>
          <col class="hzh-poi-col-city" />
          <col class="hzh-poi-col-name" />
          <col class="hzh-poi-col-feature" />
          <col class="hzh-poi-col-category" />
          <col class="hzh-poi-col-district" />
          <col class="hzh-poi-col-fit" />
          <col class="hzh-poi-col-budget" />
          <col class="hzh-poi-col-address" />
          <col class="hzh-poi-col-duration" />
          <col class="hzh-poi-col-currency" />
...
def render_poi_management(poi_df):
    ...
    with st.expander("飞书多维表格同步", expanded=False):
        st.caption("从飞书多维表格实时读取 POI。第一版为只读同步：飞书是数据源，工具会把读取到的点位合并保存到当前 POI 数据库。")
        if sync_feishu_pois is None:
            st.error("飞书同步模块未加载，请确认 feishu_poi_sync.py 已随项目部署。")
        configured = feishu_configured()
        if configured:
            st.success("已检测到飞书配置。")
        else:
            st.info("尚未配置完整飞书参数。请在本地 .streamlit/secrets.toml 或 Render 环境变量中配置 FEISHU_APP_ID、FEISHU_APP_SECRET、FEISHU_APP_TOKEN 或 FEISHU_WIKI_URL、FEISHU_TABLE_ID。")

        st.caption("当前已固定连接你提供的「城市 POI 汇总表」。下面 3 个字段通常不需要改；view_id 默认留空，会同步整张表。")
        manual_cols = st.columns(2)
        with manual_cols[0]:
            app_token_input = st.text_input(
                "飞书 wiki 链接 / 多维表格 app_token",
                value=DEFAULT_FEISHU_WIKI_URL,
                help="默认使用你提供的飞书 wiki 链接，系统会自动解析真实多维表格 app_token。",
            )
        with manual_cols[1]:
            table_id_input = st.text_input(
                "城市 POI 汇总表 table_id",
                value=DEFAULT_FEISHU_TABLE_ID,
                help="已固定为城市 POI 汇总表：tblNnZpZDhlTHAiU。",
            )
        view_id_input = st.text_input(
            "视图 view_id（可选，默认留空）",
            value=DEFAULT_FEISHU_VIEW_ID,
            placeholder="默认使用城市 POI 汇总表视图：vewt5rgo06",
            help="已固定为当前城市 POI 汇总表视图；如需同步整张表可手动清空。",
        )
        sync_mode = st.radio("同步方式", ["覆盖同编号点位", "只新增不覆盖", "替换全部现有数据"], horizontal=True, key="feishu_sync_mode")

        if st.button("从飞书同步 POI", type="primary", disabled=sync_feishu_pois is None):
            try:
                with st.spinner("正在读取飞书多维表格，并按固定字段同步 POI……"):
                    raw_sync_result = sync_feishu_pois(
                        secret_or_env("FEISHU_APP_ID"),
                        secret_or_env("FEISHU_APP_SECRET"),
                        app_token_input,
                        table_id_input,
                    ...
                    fixed_mapping = feishu_poi_fixed_mapping(raw_sync_result.source_dataframe.columns)
                    import_result = transform_source_to_poi(raw_sync_result.source_dataframe, custom_mapping=fixed_mapping)
                sync_result = raw_sync_result
                mapping_rows = [
                    {
                ...
                st.session_state.feishu_import_preview = {
                    "mapping_rows": mapping_rows,
                    "dataframe": import_result.dataframe,
                    "source_dataframe": sync_result.source_dataframe,
                    "source_columns": import_result.source_columns,
                    "record_count": sync_result.record_count,
                    "missing_required_count": import_result.missing_required_count,
                    "empty_price_count": import_result.empty_price_count,
        ...
        preview = st.session_state.get("feishu_import_preview")
        if preview:
            st.markdown("**同步预览与写入**")
            st.markdown("**固定字段映射结果**")
            st.dataframe(preview["mapping_rows"], use_container_width=True, hide_index=True)
            mapped_keys = {row.get("系统字段 key") for row in preview["mapping_rows"]}
            required_keys = {"name_cn", "city", "category"}
            missing_keys = sorted(required_keys - mapped_keys)
            if missing_keys:
                missing_labels = ", ".join(IMPORT_FIELD_LABELS.get(key, key) for key in missing_keys)
                st.warning(f"以下关键字段没有同步到：{missing_labels}。请确认飞书表里有「POI/服务名称」「城市」「分类」这些列。")
            with st.expander("查看飞书原始字段和原始数据", expanded=False):
                st.caption("这里展示飞书原始字段，方便确认固定映射是否覆盖了当前表。")
                st.write("飞书原始字段：" + "、".join(preview.get("source_columns", [])))
                source_df = preview.get("source_dataframe")
                if source_df is not None:
                    st.dataframe(source_df.head(20), use_container_width=True, hide_index=True)
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("飞书记录数", preview["record_count"])
            c2.metric("可同步点位", len(preview["dataframe"]))
            c3.metric("缺少名称", preview["missing_required_count"])
            c4.metric("价格为空", preview["empty_price_count"])
            st.markdown("**同步数据预览（前 20 条，不是全部数据）**")
            st.caption(f"本次从飞书读取 {preview['record_count']} 条记录；这里只显示前 20 条用于检查字段映射。")
            st.dataframe(display_poi_df(preview["dataframe"].head(20)), use_container_width=True, hide_index=True)
            if st.button("确认写入当前 POI 数据库", type="primary"):
                before_count = len(poi_df)
                if preview["mode"] == "替换全部现有数据":
                    final_df = preview["dataframe"]
                elif preview["mode"] == "覆盖同编号点位":
                    final_df = merge_poi_data(poi_df, preview["dataframe"], overwrite=True)
                else:
                    final_df = merge_poi_data(poi_df, preview["dataframe"], overwrite=False)
                save_pois(final_df)
                after_count = len(final_df)
                st.session_state.feishu_import_preview = None
                st.success(f"飞书 POI 同步完成：当前数据库共 {after_count} 条，变化 {after_count - before_count:+d} 条。")
                st.rerun()
    ...
    with filter_cols[0]:
        categories = st.multiselect("分类", sorted([value for value in management_df["category"].unique() if clean_text(value)]))
    with filter_cols[1]:
        cities = st.multiselect("城市", sorted([value for value in management_df["city"].unique() if clean_text(value)]))
    with filter_cols[2]:
        tags = st.multiselect("体验特色", unique_multi_values(management_df, "tags"))
    with filter_cols[3]:
        suitable_for = st.multiselect("医旅适配及注意事项", unique_multi_values(management_df, "suitable_for"))
    with filter_cols[4]:
        keyword = st.text_input("搜索 POI/服务名称")
